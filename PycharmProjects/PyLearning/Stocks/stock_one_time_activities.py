...
def get_report_data(conn, type):
    ...
    if type == 'fluc':
        return_cnt = curr.execute("select * from stocks.script_detailed_info_history where stock_date = %s "
                              "and abs(fluctuation) > 10 order by fluctuation desc", args='2018-04-09')
    elif type == "change" :
        return_cnt = curr.execute("select * from stocks.script_detailed_info_history where stock_date = %s "
                                  "and abs(per_change_today) > 10 order by per_change_today desc", args='2018-04-09')
        ...
        return_cnt = curr.execute("select * from ( select curr.script_name as script_name, "
                                  ...
                                  "on datediff(cur.mx,dl.dt)>=7)) a where week_per_change > 10 order by week_per_change")
